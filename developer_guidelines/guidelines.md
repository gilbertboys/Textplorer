...
2. `developer_guidelines/` - Contains contributor documents including this file, explaining how the project was built and how to work on it.
3. `documents/` - Project documentation including the user manual and our living document which tracks project decisions and progress over time.
4. `node_modules/` - All npm dependencies required to run the game. Generated by running `npm install` and should never be manually edited or committed to Git.
5. `parser/` - JavaScript files containing the core parsing logic that converts uploaded .txt files into structured level data the game engine can render.
6. `public/` - All client-side code served to the browser, including HTML pages, CSS stylesheets, game images, game logic, and sample level .txt files.
7. `reports/` - Weekly status reports created throughout development to document progress, completed tasks, and upcoming work.
8. `test_cases/` - All automated test files. New tests should be added here following existing naming conventions.
9. `.gitignore` - Specifies files and folders Git should not track, such as `node_modules/`.
...
12. `package.json` - Defines all project dependencies and npm scripts including `npm start` and `npm test`.
13. `server.js` - The main Express server file that handles API routes for parsing and loading levels.
